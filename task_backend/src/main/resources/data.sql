...
INSERT INTO vendors (vendor_id, business_name, business_certificate_number, billing_address)
VALUES
    (gen_random_uuid(), 'Tech Supplies Co.', 'CERT12345', '123 Tech Street, Cairo, Egypt'),
    (gen_random_uuid(), 'Fashion World', 'CERT67890', '456 Fashion Avenue, Alexandria, Egypt'),
    (gen_random_uuid(), 'Sports Gear Inc.', 'CERT11223', '789 Sports Lane, Giza, Egypt');
...
VALUES
    (gen_random_uuid(),
     (SELECT cart_id FROM carts WHERE customer_id = (SELECT customer_id FROM customers WHERE name = 'John Doe')),
     (SELECT product_id FROM products WHERE product_name = 'Laptop'),
     1),
    (gen_random_uuid(),
     (SELECT cart_id FROM carts WHERE customer_id = (SELECT customer_id FROM customers WHERE name = 'Jane Smith')),
     (SELECT product_id FROM products WHERE product_name = 'Running Shoes'),
     2),
    (gen_random_uuid(),
     (SELECT cart_id FROM carts WHERE customer_id = (SELECT customer_id FROM customers WHERE name = 'Alice Johnson')),
